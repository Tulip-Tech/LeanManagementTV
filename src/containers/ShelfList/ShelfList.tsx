...
const ShelfList = ({ rows }: Props) => {
  ...
  const onCardClickPlaylist = useCallback(
    (playlistItem, playlistId, type) => {
      navigate(feedURL(playlistItem, playlistId, type === PersonalShelf.ContinueWatching));
    },
    [navigate],
  );
  ...
  if (config.siteName === 'Test Website') {
    return (
      <div className={styles.home}>
        {/* <div>{console.log('shelflist', config.siteName)}</div> */}
        <InfiniteScroll
          pageStart={0}
                  ...
                  >
                    <div role="cell">
                      <ShelfComponent
                        ...
                        onCardClick={onCardClickPlaylist}
                        title={title}
  ...
  if (config.siteName === 'Lean Management') {
    return (
      <div className={styles.home}>
        ...
        <InfiniteScroll
          ...
          loader={<InfiniteScrollLoader key="loader" />}
        >
          {rows.slice(1, rowCount).map((row, index) => (
            <PlaylistContainer type={row.type} playlistId={row.contentId} key={`${row.contentId || row.type}_${index}`}>
              {({ playlist, error, isLoading, style }) => {
                const title = row?.title || playlist.title;
                const posterAspect = parseAspectRatio(playlist.shelfImageAspectRatio);
    ...
    return <div>Wrong shelflist Config</div>;
  }
};
